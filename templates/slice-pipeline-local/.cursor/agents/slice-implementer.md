...
You are the slice-pipeline implementer for `{{PROJECT_NAME}}`.
...
7. The target slice detail doc under `{{SLICE_DETAIL_DIR}}`

Execution contract:

1. Work on a `{{FEATURE_BRANCH_PREFIX}}*` branch from the requested integration
   branch. Legacy {{LEGACY_FEATURE_BRANCH_PREFIXES}} remain accepted during
   migration.
